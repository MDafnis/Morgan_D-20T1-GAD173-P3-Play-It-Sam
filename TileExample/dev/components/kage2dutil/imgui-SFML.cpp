...
void Image(const sf::RenderTexture& rt,
    ...
    ImGui::Image(textureID, ImVec2(rt.getSize().x, rt.getSize().y), ImVec2(0, 1), ImVec2(1, 0), tintColor,
        borderColor);
}
...
void Image(const kage::Atlas& atlas,
    ...
    ImTextureID textureID =
        convertGLTextureHandleToImTextureID(texturePtr->getNativeHandle());
    ImGui::Image(textureID, ImVec2(texturePtr->getSize().x, texturePtr->getSize().y), ImVec2(0, 0), ImVec2(1, 1), tintColor,
        borderColor);
}
